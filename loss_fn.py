...
def get_recon_loss(input_mat, output_mat):
    n = input_mat.size(0) # n must be 1 in our case
    recon_loss = F.binary_cross_entropy_with_logits(input_mat, output_mat, reduction="sum").div(n)
    return recon_loss

def vae_loss(input_mat, output_mat, mu, logvar, beta=10):
    recon_loss = get_recon_loss(input_mat, output_mat)
